...
import { useDashboard, DashboardModule } from "@/features/app/providers/DashboardProvider";
...
export function Sidebar({
  collapsed = false,
  setCollapsed,
  // activeHref removed; we will derive active state from pathname
  ...
  const { role } = useRole();
  const { setSelectedModule, selectedModule } = useDashboard();
  const router = useRouter();
  const pathname = usePathname();
  const roleItems = items === NAVIGATION_ITEMS ? (ROLE_NAVIGATION_ITEMS[role] || items) : items;

  // Mapping from sidebar label to dashboard module keys
  const labelToModule: Record<string, DashboardModule> = {
    "Ops Telemetry": "opsTelemetry",
    "AI Agents Registry": "aiAgents",
    "Gate Influx Status": "gateInflux",
      ...
      <div className="flex flex-col space-y-6">
        ...
        >
          <div
            ...
            className={cn(
              "cursor-pointer focus:outline-none focus-visible:ring-1 focus-visible:ring-arena-primary rounded-sm transition-all duration-200 select-none",
        ...
        <nav className="flex flex-col space-y-1.5 overflow-y-auto max-h-[calc(100vh-200px)]">
          {roleItems.map((item) => (
            <SidebarItem
              key={item.label}
              item={item}
              active={labelToModule[item.label] ? selectedModule === labelToModule[item.label] : pathname === item.href}
              collapsed={collapsed}
              onClick={() => {
